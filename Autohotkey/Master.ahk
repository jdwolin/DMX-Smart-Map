...
Loop Files, C:\Users\Missions Main\Dropbox\FCBC Missions Map\Videos\Live\*.mp4  ; Recurse into subfolders.
{
...
totalfiles := FileArray.MaxIndex()  ; how many files in the folder?
;msgbox Totalfiles = %totalfiles% 

Loop 
{
FileDelete, C:\Users\Missions Main\Dropbox\FCBC Missions Map\Python\filetoimport.py
totalfiles := FileArray.MaxIndex()  ; how many files in the folder?
if totalfiles < 2
break
;msgbox Total Files = %totalfiles%
Random, number, 1, % totalfiles  ; pick a random number from 1 to total number of files.
testelement := FileArray[number] 
SplitPath, testelement, , ,, noextension, ; send current file without extension
;

filecontents = filename=r'%noextension%'`n
;MsgBox, %filecontents%
FileAppend, %filecontents%, C:\Users\Missions Main\Dropbox\FCBC Missions Map\Python\filetoimport.py

Run, cmd /c python "%userprofile%\Dropbox\FCBC Missions Map\Python\Master.pyw" , , hide
sleep 1500
;msgbox test
SetWorkingDir, %userprofile%\Dropbox\FCBC Missions Map\
RunWait, cmd /c ffplay "%userprofile%\Dropbox\FCBC Missions Map\Videos\Live\%testelement%" -fs -autoexit, , hide  ; play the video
;msgbox test2
FileArray.remove(number)  ; now delete that file from the array.  
sleep 3000
}
